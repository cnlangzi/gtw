  ...
  async execute(args) {
    ...
    if (wip.pendingPr) {
      ...
      try {
        ...
        // Build PR body — LLM already includes "Fixes: #N" at start if issue-linked
        // Only append footer if body doesn't already contain a closing keyword
        let prBody = body || '';
        const hasClosingKeyword = /(?:fixes|closes|resolves):\s*#\d+/i.test(prBody);
